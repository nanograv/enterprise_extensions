...
def optimal_num_components(toas,mask_select=None):
    """ Number of components for red noise according to Lentati et al.
        'Spin noise and systematics ...' paper. 
        From 1/Tobs to 1/(60 days) with 1/Tobs spacing """
    if mask_select is None:
      mask_select = [True for toa in toas]
    toas = toas[mask_select]
...
def red_noise_block(psr=None, psd='powerlaw', prior='log-uniform', Tspan=None,
    ...
    if components == 'optimal' and select_band_names is None:
      components_all = optimal_num_components(psr.toas)
      print('Optimal components red ',components_all)
      if outdir is not None:
        with open(outdir+'n_comp_all.txt', 'w') as comp_file: comp_file.write(str(components_all))
    else:
    ...
    if psd in ['powerlaw', 'turnover', 'tprocess', 'tprocess_adapt']:
        ...
        if psd == 'powerlaw':
            ...
            df = 2
            alphas = InvGamma(df/2, df/2, size=components_all)
    ...
    if psd == 'spectrum':
        if prior == 'uniform':
            log10_rho = parameter.LinearExp(-10, -4, size=components_all)
        elif prior == 'log-uniform':
            log10_rho = parameter.Uniform(-10, -4, size=components_all)
    ...
    if select_band_names is not None:
         if components == 'optimal':
           ...
           if select_band_names=='custom':
             ...
             name_select_band = psr.flagval_bor_1
           elif select_band_names=='custom_2':
             name_select_band = psr.flagval_bor_2
           elif select_band_names=='custom_3':
             name_select_band = psr.flagval_bor_3
           else:
             name_select_band = select_band_names
           print('Optimal components ',name_select_band,' ',components_select)
           if outdir is not None:
             with open(outdir+name_select_band+'n_comp.txt', 'w') as comp_file: comp_file.write(str(components_select))
         else:
         ...
         rn = gp_signals.FourierBasisGP(pl, components=components_select,
                                   Tspan=Tspan, coefficients=coefficients,
...
def return_band_selection_class_from_name(band):
    selection_fun = return_band_selection_fun_from_name(band)
    return selections.Selection(selection_fun)

def return_band_selection_fun_from_name(band):
    ''' Return selection-by-band class from band name '''
    if band == '50cm' or band == 'band_50cm' or band == 'by_50cm':
        # selection by only 50cm data
        selection = selections.band_50cm
    if band == '40cm' or band == 'band_40cm' or band == 'by_40cm':
        # selection by only 40cm data
        selection = selections.band_40cm
    if band == '4050cm' or band == 'band_4050cm' or band == 'by_4050cm':
        # selection by only 40cm and 50cm data
        selection = selections.band_4050cm
    if band == '20cm' or band == 'band_20cm' or band == 'by_20cm':
        # selection by only 20cm data
        selection = selections.band_20cm
    if band == '10cm' or band == 'band_10cm' or band == 'by_10cm':
        # selection by only 10cm data
        selection = selections.band_10cm
    if band == '1020cm' or band == 'band_1020cm' or band == 'by_1020cm':
        # selection by only 10cm data
        selection = selections.band_1020cm
    if band == 'B10CM' or band == 'b10cm':
        selection = selections.by_b_10cm
    if band == 'B20CM' or band == 'b20cm':
        selection = selections.by_b_20cm
    if band == 'B40CM' or band == 'b40cm':
        selection = selections.by_b_40cm
    if band == 'B50CM' or band == 'b50cm':
        selection = selections.by_b_50cm
    if band == 'B4050CM' or band == 'b4050cm':
        selection = selections.by_b_4050cm
    if band == 'B1020CM' or band == 'b1020cm':
        selection = selections.by_b_1020cm
    if band == 'caspsr':
        # selection by only 10cm data
        selection = selections.system_caspsr
    if band == 'cpsr2':
        # selection by only 10cm data
        selection = selections.system_cpsr2
    if band == 'cpsr2_20cm':
        # selection by only 10cm data
        selection = selections.system_cpsr2_20cm
    if band == 'cpsr2_50cm':
        # selection by only 10cm data
        selection = selections.system_cpsr2_50cm
    if band == 'wbcorr':
        # selection by only 10cm data
        selection = selections.system_wbcorr
    if band == 'jflag_pdfb4_20cm':
        # selection by only 10cm data
        selection = selections.system_jflag_pdfb4_20cm
    if band == 'pdfb1_early':
        # selection by only 10cm data
        selection = selections.system_pdfb1_early
    if band == 'pdfb1':
        # selection by only 10cm data
        selection = selections.system_pdfb1
    if band == 'pdfb':
        # selection by only 10cm data
    ...
    if band == 'custom_1':
        selection = selections.custom_selection_1
    if band == 'custom_2':
...
def return_mask_from_selection_name(psr,band):
    """ Return mask for ToAs from band selection name ('band' variable) """
    selfunc = return_band_selection_fun_from_name(band)
    args_selfunc = inspect.getargspec(selfunc).args
    argdict = {attr: getattr(psr,attr) for attr in dir(psr) if attr in args_selfunc}
    return selfunc(**argdict)

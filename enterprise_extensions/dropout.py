...
def dp_efac_ndiag(toaerrs, efac=1.0, efac_drop=0.5, efac_thresh=0.5):
    ...
    elif efac_drop < efac_thresh:
        return toaerrs ** 2


def dropout_MeasurementNoise(efac=parameter.Uniform(0.5, 1.5),
                             efac_drop=parameter.Uniform(0, 1), efac_thresh=0.5,
                             selection=Selection(selections.no_selection), name=""):
    """Class factory for EFAC type measurement noise."""

    varianceFunction = dp_efac_ndiag(efac=efac,
                                     efac_drop=efac_drop,
                                     efac_thresh=efac_thresh)
    BaseClass = white_signals.WhiteNoise(varianceFunction, selection=selection, name=name)

    class dropout_MeasurementNoise(BaseClass):
        signal_name = "efac"
        signal_id = "efac_" + name if name else "efac"

    return dropout_MeasurementNoise


@signal_base.function
def dp_equad_ndiag(toas, log10_equad=-8, equad_drop=0.5, equad_thresh=0.5):
...
def dropout_EquadNoise(log10_equad=parameter.Uniform(-10, -5),
                       equad_drop=parameter.Uniform(0, 1), equad_thresh=0.5,
                       selection=Selection(selections.no_selection),
                       name=""):
    """Class factory for EQUAD type measurement noise."""

    varianceFunction = dp_equad_ndiag(log10_equad=log10_equad,
...
def dropout_EcorrBasisModel(
    ...
    return dropout_EcorrBasisModel

...
class JumpProposal(object):
    def __init__(self, pta, snames=None, empirical_distr=None, f_stat_file=None):
        ...
        if snames is None:
            allsigs = np.hstack([[qq.signal_name for qq in pp._signals] for pp in pta._signalcollections])
    ...
    def draw_from_empirical_distr(self, x, iter, beta):
        ...
        if self.empirical_distr is not None:
            ...
            if self.empirical_distr[distr_idx].ndim == 1:
                ...
                oldsample = [x[self.pnames.index(p)] for p in self.empirical_distr[distr_idx].param_names]
                ...
                lqxy = self.empirical_distr[distr_idx].logprob(oldsample) - self.empirical_distr[distr_idx].logprob(
                    newsample
                )
    ...
    def draw_from_psr_empirical_distr(self, x, iter, beta):
        ...
        if self.empirical_distr is not None:
            ...
            pnames = [ed.param_name if ed.ndim == 1 else ed.param_names for ed in self.empirical_distr]
            ...
            for idx in idxs:
                if self.empirical_distr[idx].ndim == 1:
                    ...
                    lqxy += self.empirical_distr[idx].logprob(x[pidx]) - self.empirical_distr[idx].logprob(q[pidx])

                else:
                    oldsample = [x[self.pnames.index(p)] for p in self.empirical_distr[idx].param_names]
                    newsample = self.empirical_distr[idx].draw()

                    for p, n in zip(self.empirical_distr[idx].param_names, newsample):
                        q[self.pnames.index(p)] = n

                    lqxy += self.empirical_distr[idx].logprob(oldsample) - self.empirical_distr[idx].logprob(newsample)
    ...
    def draw_from_gwb_log_uniform_distribution(self, x, iter, beta):
        ...
        # draw parameter from signal model
        gw_pars = [par for par in self.pnames if ("gw" in par and "log10_A" in par)]
    ...
    def draw_from_par_prior(self, par_names):
        ...
        if not par_list:
            raise UserWarning("No parameter prior match found between {} and PTA.object.".format(par_names))
    ...
    def draw_from_par_log_uniform(self, par_dict):
        ...
        for par_name in par_dict.keys():
            ...
            if pn_list:
                ...
                name_list.append(par_name)
        if not par_list:
            raise UserWarning("No parameter dictionary match found between {} and PTA.object.".format(par_dict.keys()))
        par_list = np.concatenate(par_list, axis=None)
    ...
    def draw_from_signal(self, signal_names):
        ...
        for signal_name in signal_names:
            try:
                ...
                pass
        if not signal_list:
            raise UserWarning("No signal match found between {} and PTA.object!".format(signal_names))
    ...
    def fe_jump(self, x, iter, beta):
        ...
        for param_name, new_param in zip(
            ["log10_fgw", "gwphi", "cos_gwtheta", "cos_inc", "psi", "phase0", "log10_h"],
...
def setup_sampler(pta, outdir="chains", resume=False, empirical_distr=None, groups=None):
    ...
    sampler = ptmcmc(ndim, pta.get_lnlikelihood, pta.get_lnprior, cov, groups=groups, outDir=outdir, resume=resume)

...
class JumpProposal(object):
                ...
                for key in self.emp_dist_dict_keys:
                    ...
                    if len(mask) >= 1:
                        self.empirical_distr[key] = [
                            self.empirical_distr[key][m] for m in mask
                        ]
                        # extend empirical_distr here:
                        print(
                            f"extending {key}'s empirical distributions to priors...\n"
                        )
    ...
    def draw_from_empirical_distr(self, x, iter, beta):
        ...
        if self.empirical_distr is not None and not isinstance(
            self.empirical_distr, dict
        ):
    ...
    def draw_from_hypermodel_empirical_distr(self, x, iter, beta):
        ...
        if self.empirical_distr is not None:
            ...
            if self.empirical_distr[key][distr_idx].ndim == 1:
                ...
                lqxy = self.empirical_distr[key][distr_idx].logprob(
                    x[idx]

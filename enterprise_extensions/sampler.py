...
class JumpProposal(object):
    ...
    def __init__(self, pta, snames=None,
                 empirical_distr=None, f_stat_file=None):
        ...
        if self.empirical_distr is not None:
            # only save the empirical distributions for parameters that are in
            # the model
            mask = []
        ...
        if empirical_distr is not None and self.empirical_distr is None:
            # if an emp dist path is provided, but fails the code, this helpful
            # msg is provided
            print(
    ...
    def draw_from_empirical_distr(self, x, iter, beta):
        ...
        if self.empirical_distr is not None:
            ...
            if self.empirical_distr[distr_idx].ndim == 1:
                ...
                for p, n in zip(
                        self.empirical_distr[distr_idx].param_names, newsample):
        ...
        if self.empirical_distr is not None:
            ...
            for idx in idxs:
                if self.empirical_distr[idx].ndim == 1:
                    ...
                    for p, n in zip(
                            self.empirical_distr[idx].param_names, newsample):
                        q[self.pnames.index(p)] = n
    ...
    def draw_from_par_log_uniform(self, par_dict):
        ...
        def draw(x, iter, beta):
            ...
            q[idx] = np.random.uniform(
                par_dict[par_name][0], par_dict[par_name][1])
    ...
    def fe_jump(self, x, iter, beta):
        ...
        while accepted == False:
            ...
            gw_theta = np.arccos(
                self.params[self.pimap['cos_gwtheta']].sample())
        ...
        hastings_extra_factor = self.params[self.pimap['log10_h']].get_pdf(
            log10_h_old)
        hastings_extra_factor *= 1 / \
            self.params[self.pimap['log10_h']].get_pdf(log10_h)
        hastings_extra_factor = self.params[self.pimap['phase0']].get_pdf(
            phase0_old)
        hastings_extra_factor *= 1 / \
            self.params[self.pimap['phase0']].get_pdf(phase0)
        hastings_extra_factor = self.params[self.pimap['psi']].get_pdf(psi_old)
        hastings_extra_factor *= 1 / \
            self.params[self.pimap['psi']].get_pdf(psi)
        hastings_extra_factor = self.params[self.pimap['cos_inc']].get_pdf(
            cos_inc_old)
        hastings_extra_factor *= 1 / \
            self.params[self.pimap['cos_inc']].get_pdf(cos_inc)
...
def get_parameter_groups(pta):
    ...
    for sc in pta._signalcollections:
        for signal in sc._signals:
            ind = [
                params.index(p) for p in signal.param_names if not gpars.size or p not in gpars]
            if ind:
                groups.append(ind)

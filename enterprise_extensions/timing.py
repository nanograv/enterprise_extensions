...
from enterprise.signals import deterministic_signals, gp_signals, parameter, signal_base
...
def filter_Mmat(psr, ltm_list=None):
...
def dm_block(
    ...
    if all(dmx_data["DMXEP"] != sorted(dmx_data["DMXEP"])):
        ...
        for sx in sorted(dmx_data["DMXEP"]):
            sorted_dmx_vals.append(
                dmx_data["DMX_value"][np.where(dmx_data["DMXEP"] == sx)][0],
            )
    ...
    popt, pcov = scipy.optimize.curve_fit(
        dm_funk, sorted_dmx_ep - dmepoch, sorted_dmx_vals - dmx_DMEPOCH,
    )
...
def timing_block(
        ...
        if hasattr(psr, "model"):
            ...
            for par in psr.fitpars:
                if hasattr(psr.model, par):
                    psr.tm_params_orig[par] = [
                        getattr(psr.model, par).value,
            ...
            orig_vals = {p: v for p, v in zip(psr.t2pulsar.pars(), psr.t2pulsar.vals(), strict=False)}
            orig_errs = {p: e for p, e in zip(psr.t2pulsar.pars(), psr.t2pulsar.errs(), strict=False)}
    ...
    for par in tm_param_list:
        ...
        if par in physical_tm_priors:
            if par in tm_param_dict:
                if "pmin" in physical_tm_priors[par]:
                    if prior_lower_bound < physical_tm_priors[par]["pmin"]:
                        prior_lower_bound = physical_tm_priors[par]["pmin"]
                if "pmax" in physical_tm_priors[par]:
                ...
                if par in psr.tm_params_orig:
                    ...
                    raise ValueError(f"{par} not in psr.tm_params_orig")

                if (
                    "pmin" in physical_tm_priors[par]
                    and "pmax" in physical_tm_priors[par]
                ):
                    ...
                    if val + err * prior_upper_bound > physical_tm_priors[par]["pmax"]:
                        if psr.tm_params_orig[par][-1] != "physical":
                            ...
                            prior_lower_bound = float(val + err * prior_lower_bound)
                        prior_upper_bound = physical_tm_priors[par]["pmax"]
                    elif psr.tm_params_orig[par][-1] == "physical":
                        prior_upper_bound = float(val + err * prior_upper_bound)
                elif (
                    "pmin" in physical_tm_priors[par]
                    or "pmax" in physical_tm_priors[par]
                ):
                    if "pmin" in physical_tm_priors[par]:
                        if (
                            ...
                            # Need to change lower bound to a non-normed prior too
                            prior_upper_bound = float(val + err * prior_upper_bound)
                    elif "pmax" in physical_tm_priors[par]:
                        if (

...
def cw_delay(toas, pos, pdist,
    ...
    if check:
        ...
        fstart = fgw * (1 - 256 / 5 * mc**(5 / 3) *
                        fgw**(8 / 3) * toas[0])**(-3 / 8)
        fend = fgw * (1 - 256 / 5 * mc**(5 / 3) * fgw **
                      (8 / 3) * toas[-1])**(-3 / 8)
        df = fend - fstart
    ...
    if evolve:
        ...
        phase = phase0 + 1 / 32 / \
            mc**(5 / 3) * (w0**(-5 / 3) - omega**(-5 / 3))

        if p_phase is None:
            phase_p = phase0 + 1 / 32 / \
                mc**(5 / 3) * (w0**(-5 / 3) - omega_p**(-5 / 3))
        else:
...
def compute_eccentric_residuals(toas, theta, phi, cos_gwtheta, gwphi,
    ...
    omhat = np.array(
        [-singwtheta * cosgwphi, -singwtheta * singwphi, -cosgwtheta])
...
def fdm_delay(toas, log10_A, log10_f, phase_e, phase_p):
    ...
    # Return the time-series for the pulsar
    return - A / (2 * np.pi * f) * (np.sin(2 * np.pi * f * toas +
                                           phase_e) - np.sin(2 * np.pi * f * toas + phase_p))

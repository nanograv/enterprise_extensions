...
class CompareTimingModels():
    ...
    def __init__(self, psrs, model_name='model_1', abs_tol=1e-3,
                 rel_tol=1e-6, dense=True, **kwargs):
        ...
        if dense:
            self.pta_marg = model(
                ...
                tm_marg=True,
                dense_like=True,
                **kwargs)  # marginalized model
        else:
            self.pta_marg = model(
                psrs, tm_marg=True, **kwargs)  # marginalized model
    ...
    def check_timing(self, number=10_000):
        ...
        print(
            'Sampling {0} points took {1} seconds.'.format(
                number, sample_time))
        ...
        time_marg = end - start - sample_time
        print(
            'Sampling {0} points took {1} seconds.'.format(
                number, time_marg))
        ...
        # remove sampling time from total time taken
        time_norm = end - start - sample_time
        print(
            'Sampling {0} points took {1} seconds.'.format(
                number, time_norm))

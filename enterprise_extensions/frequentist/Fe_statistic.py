...
class FeStat(object):
    ...
    def compute_Fe(self, f0, gw_skyloc, brave=False,
        ...
        for idx, (psr, Nmat, TNT, phiinv, T) in enumerate(zip(self.psrs, self.Nmats,
            ...
            A = np.zeros((4, ntoa))
            A[0, :] = 1 / f0 ** (1 / 3) * np.sin(2 *
                                                 np.pi * f0 * (psr.toas - tref))
            A[1, :] = 1 / f0 ** (1 / 3) * np.cos(2 *
                                                 np.pi * f0 * (psr.toas - tref))
            A[2, :] = 1 / f0 ** (1 / 3) * np.sin(2 *
                                                 np.pi * f0 * (psr.toas - tref))
            A[3, :] = 1 / f0 ** (1 / 3) * np.cos(2 *
                                                 np.pi * f0 * (psr.toas - tref))

            ip1 = innerProduct_rr(A[0, :], psr.residuals,
                                  Nmat, T, Sigma, brave=brave)
            ip2 = innerProduct_rr(A[1, :], psr.residuals,
                                  Nmat, T, Sigma, brave=brave)
            ip3 = innerProduct_rr(A[2, :], psr.residuals,
                                  Nmat, T, Sigma, brave=brave)
            ip4 = innerProduct_rr(A[3, :], psr.residuals,
                                  Nmat, T, Sigma, brave=brave)
        ...
        for j, gw_pos in enumerate(gw_skyloc.T):
            ...
            if maximized_parameters:
                ...
                # correcting weird problem of degeneracy (psi-->pi-psi/2 and
                # phi0-->2pi-phi0 keep everything the same)
                ...
                # related to amplitude, zeta=M_chirp^(5/3)/D
                zeta = np.abs(AA) / 4

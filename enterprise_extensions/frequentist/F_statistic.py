...
class FpStat(object):
        ...
        if pta is None:
            ...
            for p in psrs:
                if 'NANOGrav' in p.flags['pta']:
                    ...
                    s3 = s + \
                        blocks.white_noise_block(vary=False, inc_ecorr=False)
                    models.append(s3(p))
    ...
    def compute_Fp(self, fgw):
        ...
        for psr, Nmat, TNT, phiinv, T in zip(self.psrs, self.Nmats,
            ...
            for jj in range(2):
                for kk in range(2):
                    M[jj, kk] = innerProduct_rr(
                        A[jj, :], A[kk, :], Nmat, T, Sigma)
    ...
    def compute_fap(self, fgw):
        ...
        return np.sum(np.exp(n * np.log(fp0) - fp0 -
                             np.log(scipy.special.gamma(n + 1))))

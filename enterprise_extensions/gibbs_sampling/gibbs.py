...
class BayesPowerSingle(object):
    ...
    def __init__(self,
        ...
        if self.ecorr_type == 'basis':
            wn = blocks.white_noise_block(vary=self.white_vary, inc_ecorr=self.inc_ecorr, gp_ecorr=True, select = select, tnequad = tnequad)
        else:
            wn = blocks.white_noise_block(vary=self.white_vary, inc_ecorr=self.inc_ecorr, gp_ecorr=False, select = select, tnequad = tnequad)
    ...
    def update_b(self, xs):
        ...
        try:
            ...
            self._b = mean + st_solve(chol[0], np.random.normal(loc=0, scale=1, size=TNT.shape[0]),
                                    lower=True, unit_diagonal=False, overwrite_b=False, check_finite=False, trans=1)
    ...
    def sample(self, niter=int(1e4), wniters = 30, eciters = 10,
               savepath = None,
        ...
        if self.white_vary:
            ...
            isave = int(4e9) ## large number to avoid saving the white noise choice in a txt file
            thin = 1
        ...
        fp = np.lib.format.open_memmap(savepath + '/chain_1.npy', 
                            mode='w+', 
                            dtype='float32', 

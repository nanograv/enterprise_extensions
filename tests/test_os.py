...
def pta_pshift(dmx_psrs, caplog):
    ...
    gw_gamma = parameter.Constant(13.0 / 3)("gw_gamma")
    gw_pl = utils.powerlaw(log10_A=gw_log10_A, gamma=gw_gamma)
    gw_pshift = gp_signals.FourierBasisGP(
        spectrum=gw_pl, components=5, Tspan=Tspan, name="gw", pshift=True, pseed=pseed
    )
...
def test_os_pseed(dmx_psrs, pta_pshift):
    ...
    params = {
        pnm: p.sample() for pnm, p in zip(pta_pshift.param_names, pta_pshift.params)
    }
